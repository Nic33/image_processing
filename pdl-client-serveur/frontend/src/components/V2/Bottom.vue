<script setup lang="ts">
    ...
    // ces tableux nous servent pour detecter le coisement :
    var tab: [number, number][] = []; // c'est le premier tableau, chaque points sont mis dedans.
    var index = 0;

    var img_is_put : boolean = false;

    let newArray: [number, number][] = []; // deuxième tableau avec tout les points de la ligne

    let Point_de_croisement: [number, number][] = []; // troisième tableau pour un tuple de croisement

    // ce tableau nous sert à mettre tout les points où la souris clique
    var last_tab : number[] = []; 

    // ce tableau nous sert à mettre tout les points de last_tab
    var concat_last_tab : number[] = []; 
        ...
        if (canvas != null){
            ...
            if (ctx != null){
                ...
                if (tour_complet() == true){
                    ...
                    createToast({ title: 'info', description: "Un croisement a été détecté"} , {toastBackgroundColor : 'rgb(0,128,0)', type : 'info', timeout : 5000, position : 'top-center', showIcon : true});
                    ...
                    if (circle == false){
                        ...
                        createToast({ title: 'info', description: "Premier cercle ✅"} , {toastBackgroundColor : 'rgb(0,128,0)', type : 'info', timeout : 5000, position : 'top-center', showIcon : true});

                        first_circle();

                    }else{

                        createToast({ title: 'info', description: "Second cercle ✅"} , {toastBackgroundColor : 'rgb(0,128,0)', type : 'info', timeout : 5000, position : 'top-center', showIcon : true});
    ...
    //cette fonction nous permet de de reset tout les tableaux
    ...
    function reset_all(){
        ...
        concat_last_tab.splice(0, concat_last_tab.length);
    ...
    async function cut(){
        ...
        console.log("last_tab = "+ concat_last_tab);

        await api.ChangeImage(imageId.value, "Bottom" , -1 , -1, concat_last_tab); 
    ...
    function first_circle(){
        console.log("first circle saved");

        concat_last_tab = concat_last_tab.concat(last_tab);
        concat_last_tab.push(-1);

        last_tab.splice(0,last_tab.length);

    }

    function second_circle(){
        console.log("second_circle saved");
...
<div>
  ...
  <h3>Bottom Elimination</h3>
  <div id ="panel">
        
    <p>L'objectif de cette partie est de supprimer un bouton de votre visage que vous aurez sélectionné au préalable.</p>
